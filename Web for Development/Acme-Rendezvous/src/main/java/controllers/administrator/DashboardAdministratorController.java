...
public class DashboardAdministratorController extends AbstractController {
	...
	public ModelAndView display() {
		...
		result.addObject("dashboard6", new String[] {
			df2.format(s.getMean()), df2.format(s.getStdDev())
		});
		...
		if (!model.isEmpty()) {
			int i = 0;
			for (final Object servicio : model) {
		...
		final Collection<Object> managersWhitCountServicesOverAverage = this.administratorService.findManagersWhitCountServicesOverAverage();
		...
		final Collection<Object> managersWhitMoreServicesCancelled = this.administratorService.findManagersOrederdByServicesCancelled();
		final Collection<Object> topManagersWhitMoreServicesCancelled = new ArrayList<>();
		if (!managersWhitMoreServicesCancelled.isEmpty()) {
			int i = 0;
			for (final Object manager : managersWhitMoreServicesCancelled) {
		...
		final Collection<Object> distinctCartoriesOfRequestedServicesAndNumberOfRendezvouses = this.administratorService.findDistinctCartoriesOfRequestedServicesAndNumberOfRendezvouses();
		...
		final Collection<Object> serviceNumberAndCategoryNumber = this.administratorService.findServiceNumberAndCategoryNumber();
		result.addObject("serviceNumberAndCategoryNumber", serviceNumberAndCategoryNumber);
		// Dashboard D09 11.2.3 The average, the minimum, the maximum, and the standard deviation of
		// services requested per rendezvous.
		final Collection<Object> avgMaxAndMinOfRequestPerRendezvous = this.administratorService.findAverageMaxAndMinServicesReaquestedPerRendezvous();
		result.addObject("avgMaxMinOfRequestPerRendezvous", avgMaxAndMinOfRequestPerRendezvous);
		// La desviacion estandar: stdev(x) = sqrt(sum(x*x)/count(x) - avg(x)*avg(x))
		Double stdev = 0.0;
		if (!avgMaxAndMinOfRequestPerRendezvous.isEmpty()) {
			final Object[] datos = (Object[]) avgMaxAndMinOfRequestPerRendezvous.iterator().next();
			final Long sum = (Long) (((Long) datos[3] == null ? 0L : datos[3]));
			final Long count = (Long) (((Long) datos[4] == null ? 0L : datos[4]));
			final Long avg = (Long) (datos[0] == null ? 0L : datos[0]);
			stdev = count == 0. ? 0. : Math.sqrt(sum / count - avg * avg);
		}
